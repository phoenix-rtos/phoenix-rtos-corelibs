...
struct {
	...
	size_t threadCnt;
	coredump_memseg_t *memList;
	size_t memSegCnt;
	coredump_reloc_t relocs[8];
	int elfclass32;
...
static void coredump_writeBuf(const char *data, size_t len)
{
	while (state.outCur + len >= sizeof(state.outBuf) - 1) {
		...
		state.outBuf[sizeof(state.outBuf) - 1] = '\n';
...
static void coredump_begin(const char *path, int signal)
{
	state.outCur = 0;
	state.rle_last = -1;
	state.rle_count = 0;
...
static void coredump_finalize(bool addCrc)
{
	if (addCrc) {
...
static int coredump_dumpThreadNotes(coredump_thread_t *threadInfo)
{
	const __u32 zero = 0;
...
{
	coredump_thread_t *threadInfo = malloc(sizeof(coredump_thread_t) + sizeof(cpu_context_t));
	if (threadInfo == NULL) {
...
static int coredump_createFile(char *crashPath, int signal)
{
	const static size_t MAX_PATH_SIZE = 256;
	...
	snprintf(path, MAX_PATH_SIZE, "%s/%s!%d@%llu.core", state.opts->savepath, crashPath, signal % 100, now);
...
static void coredump_dump(coredump_general_t *crashInfo)
{
	if (state.opts->savepath != NULL && coredump_createFile(crashInfo->path, crashInfo->signo) != 0) {
	...
	state.memList = malloc(sizeof(coredump_memseg_t) * crashInfo->memSegCnt);
	state.threadStacks = malloc(state.threadCnt * sizeof(state.threadStacks[0]));
	if ((crashInfo->memSegCnt > 0 && state.memList == NULL) || state.threadStacks == NULL) {
		PRINT_ERR("malloc failed\n");
		goto err;
	}

	if (coredump_getMemList(sizeof(coredump_memseg_t) * crashInfo->memSegCnt, state.memList) != 0) {
	...
	coredump_begin(crashInfo->path, crashInfo->signo);
...
int coredump_serverthr(coredump_opts_t *opts)
{
	...
	state.opts = opts;
	if (state.opts->savepath != NULL) {
		int ret = initSaveDir(state.opts->savepath);
		if (ret == 0) {
			state.opts->savepath = strdup(state.opts->savepath);
		}
		else {
...
int coredump_printElf(char *path)
{
	printf("Printing ELF file: %s\n", path);
	...
	state.fp = NULL;
	state.opts = malloc(sizeof(coredump_opts_t));
	state.opts->print = 1;
	state.opts->printSleep = 10 * 1000; /* 10ms */

	int signo = 0;
	char *signal = strrchr(path, '!');
	if (signal != NULL) {
		*(signal++) = '\0';
		signo = atoi(signal);
	}
	coredump_begin(strrchr(path, '/'), signo);

	__u8 buff[COREDUMP_OUTBUF_SIZE];
	int ret;
	while ((ret = fread(buff, 1, sizeof(buff), f)) > 0) {
		coredump_encodeChunk(buff, ret);
	}

	coredump_finalize(false);
