...
typedef struct _graph_t {
  ...
  int (*open)(struct _graph_t *graph, u8 irq);
  ...
  int (*line)(struct _graph_t *graph, char *arg);
...
extern int graph_init();


/* Function prepares library for use */
extern int graph_open(unsigned int mem, u8 irq = 255);

/* Function closes library */
extern int graph_close();
...
extern int graph_cursorshow();

extern int graph_cursorhide();


extern char *graph_version();

extern int graph_addr(unsigned int x, unsigned int y);

extern int graph_span();

/* Function returns free frame buffer memory offset */
extern int graph_offset();
