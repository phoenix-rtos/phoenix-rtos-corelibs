...
#if PERF_RTT_ENABLED
static int initChannels(trace_ctx_t *ctx, size_t bufsize, const char *outputDir) { return EOK; }


static size_t readChannels(trace_ctx_t *ctx) { return 0; }
#else
static int initChannels(trace_ctx_t *ctx, size_t bufsize, const char *outputDir)
{
	...
	ctx->chans = calloc(ctx->nchans, sizeof(chan_t));
	if (ctx->chans == NULL) {
	...
	for (size_t i = 0; i < ctx->nchans / trace_channel_count; i++) {
		for (size_t j = 0; j < trace_channel_count; j++) {
			chan_t *chan = &ctx->chans[j + trace_channel_count * i];
...
static int startTrace(trace_ctx_t *ctx, bool rolling)
{
	...
	if (ctx == NULL || ctx->state != trace_state_init) {
		return -EINVAL;
	}
